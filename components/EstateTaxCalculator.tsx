...
export const EstateTaxCalculator: React.FC = () => {
  ...
  const FixedToggle = ({ label, subtext, tooltip }: { label: string, subtext?: string, tooltip?: string }) => (
    <div
        ...
        <div className="flex items-center gap-2">
          ...
          {tooltip && (
            <Tooltip content={tooltip}>
              <Info size={16} className="text-gray-400 hover:text-primary cursor-help" />
            </Tooltip>
          )}
        </div>
        {subtext && <span className="text-xs text-gray-400">{subtext}</span>}
      ...
      <div className="lg:col-span-8 space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8 relative overflow-hidden">
          ...
          <div className="space-y-8">
            <section>
              <h3 className="text-sm font-bold text-textMain mb-2">遺產總額</h3>
              <Input
                label=""
                value={totalAssets}
                ...
                aria-label="請輸入遺產總市值(包含不動產、存款、股票、保單價值等)"
              />
              <div className="flex flex-wrap gap-2 mt-2">
                <button
                  onClick={() => setTotalAssets('')}
                  className="px-3 py-1.5 text-sm bg-gray-100 text-red-500 font-medium rounded-md hover:bg-gray-200 transition-colors border border-gray-200"
                >
                  清除數字
                </button>
                {[
                  { label: '1千萬', value: 10000000 },
                  { label: '3千萬', value: 30000000 },
                  { label: '5千萬', value: 50000000 },
                  { label: '1億', value: 100000000 },
                ].map((item) => (
                  <button
                    key={item.value}
                    onClick={() => setTotalAssets(item.value.toString())}
                    className="px-3 py-1.5 text-sm bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors border border-gray-200"
                  >
                    {item.label}
                  </button>
                ))}
            ...
            <section>
              <h3 className="text-sm font-bold mb-3 text-textMain">扣除額項目勾選</h3>
        ...
        <TaxRateReference currentTaxableAmount={result ? result.taxableAmount : 0} />
